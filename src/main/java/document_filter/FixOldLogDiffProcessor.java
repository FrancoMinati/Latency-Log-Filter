...
public class FixOldLogDiffProcessor {

    // Patrones para log timestamp (con y sin corchetes)
    private static final Pattern logTimePatternBracket = Pattern.compile("\\[(\\d{4}/\\d{2}/\\d{2}-\\d{2}:\\d{2}:\\d{2},\\d{3})\\]");
    private static final Pattern logTimePatternNoBracket = Pattern.compile("^(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{3})");

    // Patrón para timestamp FIX
    ...
    private static final DateTimeFormatter logFormatterBracket = DateTimeFormatter.ofPattern("yyyy/MM/dd-HH:mm:ss,SSS");
    private static final DateTimeFormatter logFormatterNoBracket = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss,SSS");
    ...
    public static void parseLog() {
        ...
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(inputPath));
            ...
            while ((line = reader.readLine()) != null) {
                ...
                Matcher logMatcher = logTimePatternBracket.matcher(line);
                DateTimeFormatter currentLogFormatter = logFormatterBracket;

                if (!logMatcher.find()) {
                    logMatcher = logTimePatternNoBracket.matcher(line);
                    currentLogFormatter = logFormatterNoBracket;
                }
                ...
                if (logMatcher.find() && fixMatcher.find()) {
                    ...
                    try {
                        LocalDateTime logTime = LocalDateTime.parse(logTimeStr, currentLogFormatter);
